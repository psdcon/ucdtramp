...
var cluster = {
    shellac: function (object, styles) { //style getter for both standards based browsers and that other browser

        function getStyle(styleProp) {
        	var style = null;
            if (object.currentStyle) 
            	style = object.currentStyle[styleProp];
            else if (window.getComputedStyle) 
            	style = document.defaultView.getComputedStyle(object, null).getPropertyValue(styleProp);
        ...
        context.font = styles.weight + " " + styles.style + " " + styles.size + "px " + styles.font; //split up the string into words
        var text = object.innerHTML;
        var wordArray = text.split(" ");
        var phraseArray = [];
        var lastPhrase = ""; //break ups
        for (var i = 0; i < wordArray.length; i++) {
            var word = wordArray[i];
            var measure = context.measureText(lastPhrase + " " + word).width;
            if (i === 0) {
                lastPhrase = word;
            } else if (measure < object.clientWidth) {
                lastPhrase += (" " + word);
            } else {
        ...
        phraseArray.push(lastPhrase); //draw text
    ...
    Node: function (x, y, parent) {
        ...
        t.grow = function (parent, evt) {
            ...
            that.timer = window.setInterval(this.loop, 40);
        };
    ...
    speed: 1000,
    timeout: -700,
    colors: [] //["#ff4040","#2482ff","#ffd20f"]
};
cluster.init('clusterMe');